# 这是"微调模型版接口 v3"（第四周 Day20 · 与 v2 只差两处常量：ADAPTER_PATH / MODEL_NAME）
# -*- coding: utf-8 -*-
...
【Day20 v3 说明：它和 day19\local_api_lora_v2.py 的关系】
    本文件 = `第四周\day19\local_api_lora_v2.py` 的**逐字复制件**，只改了【2 个常量】：
    ADAPTER_PATH : ...\第四周\day19\lora_adapter_v2  → ...\第四周\day20\lora_adapter_v3（换 adapter）
    MODEL_NAME   : "Qwen2.5-3B-Instruct-LoRA-v2"     → "Qwen2.5-3B-Instruct-LoRA-v3"（换招牌名）
**其余一字不动**——这样 v2/v3 两个服务的唯一差别就是"加载了哪个 adapter"，
F3 拆变量轮（真实 201 vs 真实 201 + 合成 99）才能把差异归因到"数据来源"这一个变量上。
⚠ **为什么必须改 MODEL_NAME**：`eval_v2.py` 的 `check_service()` 会做**服务身份检查**
   （对比 `/v1/models` 返回的名字与 profile 要求的名字）。若 v2/v3 都叫同一个名字，
   你会永远不知道"刚跑的这一轮到底是哪个 adapter"——这是第三周就立下的"防跑错模型"纪律。
...
- Windows 下 uvicorn 命令要在 (llm) conda 环境里执行；若提示找不到 uvicorn，
  改用  python -m uvicorn local_api_lora_v3:app --host 127.0.0.1 --port 8000
- 模型加载约 1~3 分钟，看到日志里出现"模型已就绪"才算启动成功。
- 别加 --reload：它会在你每次保存代码时重新执行 lifespan、重新加载 3B 模型。
- ⚠ 起服务前先确认训练已结束、显存已释放（6G 显存一次只能装一个 3B）。
"""
...
# ↑ ★ Day13 新增导入：从 peft（HuggingFace 的高效微调库）引入 PeftModel。
#   PeftModel.from_pretrained(底座模型, adapter路径) 能把 Day12 微调的 LoRA adapter
#   "挂"到 4bit 底座上，推理时自动计算 W0 + B·A。不加这一行，下面用 PeftModel 会报 NameError。
...
# ⭐ Day20 改动 1/2：指向 day20 新训的 adapter（v1/v2 的 adapter 保持不动）
ADAPTER_PATH = r"D:\Lan\研究生\技术学习\大模型算法\第四周\day20\lora_adapter_v3"
# ⭐ Day20 改动 2/2：换招牌名（防"服务在跑 ≠ 服务正确"——eval_v2 会核对这个名字）
MODEL_NAME = "Qwen2.5-3B-Instruct-LoRA-v3"  # 对外暴露的模型名（OpenAI 请求里的 model 字段）
# ↑ 对外"招牌名字"。调用方在请求里写 "model": "Qwen2.5-3B-Instruct-LoRA-v2" 就会被认出来。
...
async def lifespan(app: FastAPI):
    ...
    # =====================================================================
    # ★ Day13 新增：加载底座之后、接客之前 —— 挂上 Day12 微调的 LoRA adapter
    #   （Day20 v3：这里加载的是 day20\lora_adapter_v3）
    # =====================================================================
    if not os.path.exists(ADAPTER_PATH):
        # ↑ 防御检查：os.path.exists(路径) 返回"这个路径存不存在"。
        #   not 表示取反 = "如果 adapter 文件夹不存在"就主动报错，防止没训就跑、报看不懂的错。
        raise RuntimeError(f"找不到 adapter 文件夹：{ADAPTER_PATH}，请先完成 Day12 训练。")

    chat_model = PeftModel.from_pretrained(chat_model, ADAPTER_PATH)
    # ↑ ★ 最核心的一行：PeftModel.from_pretrained(底座模型, adapter路径)
    #   - 读 adapter_config.json（r=8 alpha=16 等配置）+ adapter_model.safetensors（B·A 权重）；
    #   - 把 B·A 增量"挂"到上面刚加载好的 4bit 底座上，返回"微调后的模型"（还是同一个变量名 chat_model）；
    #   - 之后 generate() 调用时 PEFT 自动算 W0 + B·A，不需要手动合权重；
    #   - ⚠ 底座必须用与训练时同款 4bit 配置加载（上面 quant_config 和 Day12 完全一致），
    #     否则 adapter 的 shape 对不上会报错（Day12 避坑表第 6 条）。
    chat_model.eval()
    # ↑ 挂完 adapter 后保险起见再切一次"评估（推理）模式"。
    print(f"[OK] LoRA adapter 已加载：{ADAPTER_PATH}")
    # ↑ 启动日志里出现这一行 = adapter 挂载成功（教程第 4.3 步靠它确认）。
    #   ⚠ v1/v2/v3 对照时**必须看这一行**确认加载的是 lora_adapter_v3，而不是 v1/v2。
...
#   ⚠ uvicorn 的命令 "uvicorn local_api_lora_v3:app" 要找的正是这个 app 对象：
#   local_api_lora_v3 = 文件名，app = 这一行创建的变量名。名字对不上就启动失败。
    ...
    return {
        ...
        "data": [
...

            {
                ...
                "id": MODEL_NAME,
                # ↑ 模型 ID：用全局常量 MODEL_NAME（v2 = Qwen2.5-3B-Instruct-LoRA-v2）。
                #   ⭐ eval_v2.py 的 check_service() 就是拿这个字段做"服务身份检查"。
...
def chat_completions(request: ChatRequest):
    ...
    try:
        ...
        #   ⚠ Day19 踩坑：`requests.raise_for_status()` 只显示 "500 Server Error"，
        #   真正的原因藏在响应的 detail 里 → 客户端要打印 r.text（见 rewrite_queries.py）。
