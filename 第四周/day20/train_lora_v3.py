...
"""
train_lora_v3.py —— QLoRA 微调训练脚本（第四周 Day20 第 3 步 · F3 **拆变量轮**）
========================================================================
【它和 day19\train_lora_v2.py 的关系】
    本文件 = `第四周\day19\train_lora_v2.py` 的**逐字复制件**，只改了【3 个常量】：
        DATA_PATH       : "sft_data_v2_real.json" → "sft_data_v2_mix.json"（换数据）
        OUTPUT_ADAPTER  : "lora_adapter_v2"       → "lora_adapter_v3"      （不覆盖 v1/v2）
        LOG_DIR         : "./train_logs_v2"       → "./train_logs_v3"      （不覆盖 v1/v2 日志）
    **其余一字不动**——超参（r/alpha/dropout/lr/epochs/batch/seed）与 v1/v2 完全相同，
    这样 v1 / v2 / v3 三组对照才**只有"训练数据"这一个变量不同**。

【三组对照在干什么（day19 留下的负结果要在这里拆开）】
    v1 = 合成 300 条        → R3，F1 0.824        （最好）
    v2 = 真实 201 条        → F3v2，F1 0.333      （退化，但"来源"和"数量"同时变了）
    v3 = 真实 201 + 合成 99 = 300 条 → 本脚本训练 （**把"数量"对齐，只剩"来源"这一个变量**）
    → 若 v3 回到 R3 水平：退化主要来自**样本少了 99 条**（数量效应）；
      若 v3 仍然退化：才能说"这批真实数据与评测集分布不匹配"（来源效应）。

【五件套】（脚本在机器学习里的哪一环 = 训练环节）
    - 模型   ：Qwen2.5-3B-Instruct，4bit 量化（NF4）底座 + LoRA 适配器（r=8, alpha=16）
    - 数据   ：Day20 的 sft_data_v2_mix.json（201 真实 + 99 合成 = 300，由 build_mix_data.py 生成），
...
【运行前】⚠ 必须先停掉 8000 端口的模型服务（6G 显存一次只能装一个 3B），
并 `nvidia-smi` 确认显存已释放；本脚本不下载新模型（复用 Day6 的本地权重）。
⚠ 必须在 `第四周\day20\` 目录下运行（`DATA_PATH` 是相对路径）。

【怎么看懂这个脚本（先读这里）】
一句话：把 Qwen 模型读进显存 → 挂上"小补丁"（LoRA）→ 喂数据
       → 更新补丁参数 → 保存补丁。
...
# —— 第二件：数据 ——
# ⭐ Day19 改动 1/3：换成 Day19 生成的真实/混合数据（原来吃 day12 的 sft_data.json 300 条合成）
DATA_PATH = "sft_data_v2_mix.json"   # ⚠ 相对"当前工作目录"→ 必须在 day20\ 目录下运行本脚本
...
# —— 第五件：训练 ——
EPOCHS = 3                    # 把数据完整过 3 遍（⚠ 条数变了，总步数也随之变）
BATCH_SIZE = 2                # 每步 2 条（6G 显存小，batch 不能大）
GRAD_ACCUM = 8                # 梯度累积 8 步，等效 batch = 2 × 8 = 16
# ⭐ Day19 改动 2/3：换输出目录名，避免覆盖 v1 的 adapter
OUTPUT_ADAPTER = "lora_adapter_v3"   # 只保存适配器到这里（不进 git）
# ⭐ Day19 改动 3/3：换日志目录，避免覆盖 v1 的训练日志
LOG_DIR = "./train_logs_v3"   # 训练日志目录
...
def main():
    ...
    # 目标：把 JSON 数据 → 切分训练/验证 → 拼成 ChatML → 转数字编号
    ...
    #   split 是一个字典：split["train"] = 训练集，split["test"] = 验证集。
    ...
    args = TrainingArguments(
        ...
        num_train_epochs=EPOCHS,         # 数据刷几遍（3）
...
if __name__ == "__main__":
    ...
    #   - 当执行 `python train_lora_v3.py` 时，__name__ = "__main__" → 执行 main()；
    #   - 当被别的脚本 `import train_lora_v3` 时，__name__ = 模块名 → 不自动跑。
