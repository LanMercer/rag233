...
【Day20 复制件说明 · 必读】本文件 = `第四周\day19\eval_v2.py` 的**逐字复制件**
（MD5 `9ef05c8b04d491a10151f4a70017d849`，可自行核对），**一行逻辑都没改**。为什么要复制一份：
    ① 让今天新跑的两轮评测（**F3v3 拆变量轮** / **F4 原模型对照**）的 `--out` 默认落在 `第四周\day20\` 下，
       与 `day20\collect_finals.py` 里内置的结果目录路径一致（否则结果散在 day19，账就不好对）；
    ② 保持"**尺子只有一把**"这条纪律的**可核验性**：跨天比较时，先证明两个脚本逐字节相同，
       再说"口径没变"——这比口头保证强（day17→day19 一直用这招）。
⚠ **不要**在今天这份脚本里改判据 / 改模板 / 改拒答词表：那会让 day19 与 day20 的数字**不可比**。
   要改尺子，就开新的一天、单独跑一轮、单独记账（并同步重跑基线）。
