# 这是"微调模型版接口 v2"（第四周 Day19 · 与 v1 只差两处常量：ADAPTER_PATH / MODEL_NAME）
# -*- coding: utf-8 -*-
...
【Day19 v2 说明：它和第三周 day13\\local_api_lora.py 的关系】
本文件 = `第三周\day13\local_api_lora.py` 的**逐字复制件**，只改了【2 个常量】：
    ADAPTER_PATH : ...\\第三周\\day12\\lora_adapter   → ...\\第四周\\day19\\lora_adapter_v2（换 adapter）
    MODEL_NAME   : "Qwen2.5-3B-Instruct-LoRA"    → "Qwen2.5-3B-Instruct-LoRA-v2"（换招牌名）
**其余一字不动**——这样 v1/v2 两个服务的唯一差别就是"加载了哪个 adapter"，
对照实验（F3）才能把差异归因到"训练数据"这一个变量上。
⚠ **为什么必须改 MODEL_NAME**：`eval_v2.py` 的 `check_service()` 会做**服务身份检查**
   （对比 `/v1/models` 返回的名字与 profile 要求的名字）。若 v1/v2 都叫同一个名字，
...
- Windows 下 uvicorn 命令要在 (llm) conda 环境里执行；若提示找不到 uvicorn，
  改用  python -m uvicorn local_api_lora_v2:app --host 127.0.0.1 --port 8000
...
# ⭐ Day19 改动 1/2：指向 day19 新训的 adapter（v1 的 lora_adapter 保持不动）
ADAPTER_PATH = r"D:\Lan\研究生\技术学习\大模型算法\第四周\day19\lora_adapter_v2"
# ⭐ Day19 改动 2/2：换招牌名（防"服务在跑 ≠ 服务正确"——eval_v2 会核对这个名字）
MODEL_NAME = "Qwen2.5-3B-Instruct-LoRA-v2"  # 对外暴露的模型名（OpenAI 请求里的 model 字段）
...
async def lifespan(app: FastAPI):
    ...
    #   （Day19 v2：这里加载的是 day19\lora_adapter_v2）
    # =====================================================================
    ...
    #   ⚠ v1/v2 对照时**必须看这一行**确认加载的是 lora_adapter_v2，而不是 v1。
...
#   ⚠ uvicorn 的命令 "uvicorn local_api_lora_v2:app" 要找的正是这个 app 对象：
#   local_api_lora_v2 = 文件名，app = 这一行创建的变量名。名字对不上就启动失败。
