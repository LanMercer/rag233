...
| F3 真实 vs 合成对照微调（复用 `train_lora.py` 配置，训 `lora_adapter_v2/`；显存串行） | 第 5 步（`train_lora_v2.py` + `sft_data_v2_mix.json` + 双 adapter 对照） |
...
| `sft_data_v2_mix.json`                     | 数据（今天生成）                                         | **F3 训练集**：真实（论文 90 + 行业 N）+ 合成 300 的**配比版**，训练脚本直接吃它                                                                                                  |
...
python eval_v2.py --top-k 8 --runs 3 --rewrite-cache queries_rewritten.json --rewrite-mode term `
  --out result_rewrite_term_runs3 --exp-id R4 --date 9/23 `
  --note "O1-R4 模型自动改写（term 档），其余同 R3" `
  --conclusion "" --append-log
```

> ⚠ `--date` **一定写你的实际执行日**（日期口径）——**本教程示例里的 `9/23` 是计划日，照抄就会污染日志的日期列**：9/20 实跑时 R4 行填对了（9/20），**R4b / R4c 两行却照抄成了 `9/23`**，事后已按目录时间戳 + `run_log.txt` 改回 **9/20**（详见实验日志的「日期口径第二次生效」注）。**记住：`--date` 是"跑的时候填"，不是"抄模板"。** 另外：**PowerShell 里不要留** `<占位符>`（`<` 是保留运算符，python 根本不会启动——day18 踩过）。
...
python eval_v2.py --top-k 8 --runs 3 --rewrite-cache queries_rewritten.json --rewrite-mode hyde `
  --out result_rewrite_hyde_runs3 --exp-id R4b --date 9/23 `
...
python eval_v2.py --top-k 8 --runs 3 --rewrite-cache queries_rewritten.json --rewrite-mode both `
  --out result_rewrite_both_runs3 --exp-id R4c --date 9/23 `
...
python eval_v2.py --top-k 8 --runs 3 --template v2 --out result_template_v2_runs3 `
  --exp-id R5 --date 9/23 --note "O2-G2 模板 09 v2（弱化拒答+禁止编造）" --append-log

# R5a：只弱化拒答（v2a）
python eval_v2.py --top-k 8 --runs 3 --template v2a --out result_template_v2a_runs3 `
  --exp-id R5a --date 9/23 --note "消融：只弱化拒答条件（治 FP）" --append-log

# R5b：只加引用/数字约束（v2b）
python eval_v2.py --top-k 8 --runs 3 --template v2b --out result_template_v2b_runs3 `
  --exp-id R5b --date 9/23 --note "消融：只加禁止编造编号/数字（治 FN）" --append-log
```
...
> **今天的推荐 = C（最干净）+ B（最能说"真实有用"）二选一**；**时间只够一个就做 C**——因为 C 是**单变量对照**（同为 300 条，只有"来自哪"不同），归因最干净、最好写报告。**B 更好看但归因复杂**（要说明"这里同时改了两件事"）。
...
python eval_v2.py --profile lora_v2 --top-k 8 --runs 3 --out result_lora_v2_runs3 `
  --exp-id F3v2 --date 9/23 --note "F3 真实数据 adapter v2（与 v1 只差训练数据）" --append-log
```

**对照表**（**检索层两列应该完全一致**——因为库、k、查询都没变，**这也是自检信号**）：


| 层   | 指标                                | v1（合成 300 条）            | v2（真实/混合 N 条）    | 差异  |
| --- | --------------------------------- | ----------------------- | ---------------- | --- |
| 检索  | strict / loose                    | 4/10 / 5/10（= R3）       | ? / ?（**应完全相同**） | 0   |
| 生成  | in 正确率                            | 4/10                    | ?                | ?   |
| 防幻觉 | out 正确率                           | 7/10                    | ?                | ?   |
| 防幻觉 | F1（回 `run_log.txt` 抄 TP/FP/FN/TN） | 0.824（TP7 FP0 FN3 TN10） | ?                | ?   |
| 总   | 总正确率                              | 11/20                   | ?                | ?   |


> **诚实提示（写进报告）**：若提升不明显甚至持平，**如实写**——"**190 条真实数据 + 同配置下与 300 条合成数据打平**"本身就是一个结论（说明**在这个规模下数据来源不是瓶颈、或数据量更重要**）。**HR 要的是"数据真实"，不是"数据一定带来分数提升"**；硬编一个"提升"才是自毁证据链。
...
| 轮次              | 改了什么            | 检索 strict      | 检索 loose   | 生成（in）         | 防幻觉（out）       | 防幻觉 F1    | 总正确率            | 结果目录                             |
| --------------- | --------------- | -------------- | ---------- | -------------- | -------------- | --------- | --------------- | -------------------------------- |
| 原始基线（第三周 day14） | ——              | 2/10 = **20%** | 3/10 = 30% | 2/10 = **20%** | 6/10 = **60%** | **0.667** | 8/20 = **40%**  | `第三周\day14\`                     |
| B0r3            | 判据校准 + 投票复跑     | 2/10           | 3/10       | 2/10           | 6/10           | 0.706     | 8/20            | `day17\result_lora_k4_runs3`     |
| **R3**（k=8 定稿行） | TOP_K 4→8、关前缀   | **4/10 = 40%** | 5/10 = 50% | **4/10 = 40%** | 7/10           | **0.824** | **11/20 = 55%** | `day17\result_lora_k8_runs3`     |
| R4（改写）          | + 查询改写（档位 __）   | __             | __         | __             | __             | __        | __              | `day19\result_rewrite_*`         |
| R5（模板）          | 模板 09 v2        | __             | __         | __             | __             | __        | __              | `day19\result_template_v2_runs3` |
| F3v2（数据）        | 真实数据 adapter v2 | __             | __         | __             | __             | __        | __              | `day19\result_lora_v2_runs3`     |
...
| O1-R4    | 查询改写（真实现）               | R3 → R4         | 40% → __%           | 55% → __%           | __                       |
...
| F3       | 真实数据 adapter v2         | R3 → F3v2       | 不变                  | 55% → __%           | __                       |
...
| 真实 vs 合成对照（F3）          | v1 vs v2 见 6.2 表                               | 同评测集、同配置、只改训练数据                                                             |
...
- [ ] `优化成果报告.md` 骨架成型（四张表 + 结论与下一步 + 日志索引）
- [ ] 表一/表二里**已跑出的数字全部回填**（R3 及之前），未跑的留 `__`
- [ ] 失败尝试记录 ≥4 条（含 RRF 负结果与今天的改写档位结论）
- [ ] 报告中明确写出"**HyDE 的 100% 是上界、不是成绩**"这一句
...
| eval_v2（day19 版）                        | `day19\eval_v2.py`（含 `--rewrite-cache` / 模板 v2a/v2b） | ☐   |
| 改写轮次结果（R4 / 档位 __）                      | `day19\result_rewrite_*`                             | ☐   |
...
| F3 对照结果（v2 adapter）                     | `day19\result_lora_v2_runs3`                         | ☐   |
| 优化成果报告（初稿）                              | `day19\优化成果报告.md`                                    | ☐   |
| 实验日志更新（R4/R5/F3 行 + Day19 诊断结论 + 新失败尝试） | `第四周\实验日志.md`                                        | ☐   |
...
```powershell
git add .
git commit -m "Day19: O1-R4 查询改写落地（R4: 检索 strict __/10）+ 模板09 v2 与逐句消融（R5/R5a/R5b）+ F2 真实行业QA 111条可追溯（市场32/产品27/公司24/标准14/论文14） + F3 真实vs合成对照微调（adapter v2）+ 优化成果报告初稿"
```
...
| F3 真实 vs 合成对照微调（复用训练配置，训 `lora_adapter_v2/`；显存串行）       | `train_lora_v2.py` + 双 adapter 同评测集对照（第 5 步）                    | ⏳      |
...
| O3《优化成果报告》初稿                                            | 四张表骨架 + 已跑数字回填（第 6 步）                                           | ⏳      |
| （day18 明日安排）把 HyDE 做成"模型自动改写"、计入成绩                      | `rewrite_queries.py` + `eval_v2.py --rewrite-cache` → R4（第 1 步） | ⏳      |
| （day18 明日安排）改写后再融合 / 评估 Rerank                          | `retrieval_lab.py --query-mode file` 复筛 + `rrf_k` 调参（第 2 步）     | ⏳      |
| （day18 明日安排）拿 R3 当新对照基线                                 | 全程同 k=8 / qi=off / runs=3，只改一个变量                                | ✅ 纪律已立 |
| 验收：真实数据 ≥100 条可追溯；v2 adapter 训练完成且显存正常；模板 v2 出对比；报告初稿成型 | 见「今日验收清单」                                                       | ⏳      |
...
| 改写真实现（R4）            | strict __/10 ｜ 总 __/20                                                                                                   | `day19\result_rewrite_*`（**可引用**，跑完回填）               |
| 模板 v2 消融（R5/R5a/R5b） | 检索列应与 R3 **完全一致**；生成/防幻觉/F1 __                                                                                           | `day19\result_template`_*                            |
...
| F3 v1 vs v2          | 检索两列应相同；生成/防幻觉/F1 __ vs __                                                                                               | `day19\result_lora_v2_runs3`                         |
