...
"""
train_lora_v2.py —— QLoRA 微调训练脚本（第四周 Day19 第 5 步 · F3 真实 vs 合成对照）
========================================================================
【它和第三周 day12\train_lora.py 的关系】
    本文件 = `第三周\day12\train_lora.py` 的**逐字复制件**，只改了【3 个常量】：
        DATA_PATH       : "sft_data.json"        → "sft_data_v2_real.json"（换数据）
        OUTPUT_ADAPTER  : "lora_adapter"         → "lora_adapter_v2"       （不覆盖 v1）
        LOG_DIR         : "./train_logs"         → "./train_logs_v2"        （不覆盖 v1 日志）
    **其余一字不动**——这是"对照实验只改一个变量（训练数据）"的硬要求：
    超参（r/alpha/dropout/lr/epochs/batch/seed）全部保持 v1 原值，否则涨跌都说不清是谁的功劳。
    ⚠ 不要原地改第三周的 train_lora.py：v1 adapter 与它的训练记录必须可复现。

【五件套】（脚本在机器学习里的哪一环 = 训练环节）
    - 模型   ：Qwen2.5-3B-Instruct，4bit 量化（NF4）底座 + LoRA 适配器（r=8, alpha=16）
- 数据   ：Day19 的 sft_data_v2_real.json（真实数据版：论文 90 条 + 行业真实条，
          或"真实 + 合成"混合版，由第 5.2 节的 one-liner 生成），
...
DATA_PATH = "sft_data_v2_real.json"   # ⚠ 相对"当前工作目录"→ 必须在 day19\ 目录下运行本脚本
...
OUTPUT_ADAPTER = "lora_adapter_v2"   # 只保存适配器到这里（不进 git）
# ⭐ Day19 改动 3/3：换日志目录，避免覆盖 v1 的训练日志
LOG_DIR = "./train_logs_v2"   # 训练日志目录
...
if __name__ == "__main__":
    ...
    #   - 当执行 `python train_lora_v2.py` 时，__name__ = "__main__" → 执行 main()；
    #   - 当被别的脚本 `import train_lora_v2` 时，__name__ = 模块名 → 不自动跑。
