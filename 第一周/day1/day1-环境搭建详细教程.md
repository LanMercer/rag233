...
> ⚠️ **重要发现**：你的显存是 **6GB**，而周计划里写的是"8G+ 显存"。6GB 也能跑，但第 6 天部署 Qwen2.5-7B 时会比较紧张，到时候我们要么用更小的模型（比如 Qwen2.5-3B），要么加几个参数硬跑。**今天不用管，先记着这个事，第 5 天我们再商量。**
...
> 这个迷你模型（tiny-random-gpt2）约 34MB，是"发动机点火测试"用的，不追求说得漂亮（它是随机初始化的参数，生成的文字可能是乱码，**这完全正常**），**能跑通就说明整个链路 OK**，等第 6 天我们再换真正的中文大模型 Qwen。
...
> 注意：这个设置只对当前终端窗口有效，下次新开终端要重新设。第 6 天部署 Qwen 时我们再系统处理镜像问题。
...
3. 你的显卡是 3060 Laptop 6GB 版，第 6 天部署 Qwen 时我会帮你选合适方案（大概率用 Qwen2.5-3B 或 7B 的 4bit 量化+调参），现在不用操心。
